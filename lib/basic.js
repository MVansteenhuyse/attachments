...
const { stripQueryParams, extractIdFromCqn } = require('./helper')
...
module.exports = class AttachmentsService extends cds.Service {
  ...
  async nonDraftHandler(req, attachment, srv) {
    ...
    if (urlWithoutParams?.endsWith("/content")) {
      const cqn = cds.odata.parse(urlWithoutParams, { service: srv })
      const extractedInfo = extractIdFromCqn(cqn)
      
      if (!extractedInfo) {
        logConfig.withSuggestion('error',
          'Could not extract ID from request URL', null,
          'Check URL format and ensure ID parameter is present',
          { url: urlWithoutParams, cqn: JSON.stringify(cqn, null, 2) })
        throw new Error('ID not found in request URL')
      }

      const { id: IDval, isSingleAttachment } = extractedInfo

      // Build query based on attachment scenario
      const queryKey = isSingleAttachment ? { up__ID: IDval } : { ID: IDval }

      // Fetch existing metadata from database to get filename and other details
      logConfig.debug('Fetching existing attachment metadata', { 
        attachmentId: IDval, 
        isSingleAttachment, 
        queryKey 
      })
      const existingRecord = await SELECT.from(attachment, queryKey).columns("filename", "mimeType", "url", "ID")
      
      if (!existingRecord) {
        logConfig.withSuggestion('error',
          'Attachment record not found in database', null,
          'Ensure attachment metadata was created before uploading content',
          { attachmentId: IDval, attachmentEntity: attachment.name, queryKey })
        throw new Error(`Attachment with ID ${IDval} not found`)
      }

      logConfig.debug('Retrieved existing attachment metadata', {
        attachmentId: IDval,
        actualAttachmentId: existingRecord.ID,
      ...
      const data = { 
        ID: existingRecord.ID, // Use the actual attachment ID from the database
        content: req.content,
        filename: existingRecord.filename,
        mimeType: existingRecord.mimeType,
  ...
  registerUpdateHandlers(srv, entity, target) {
    srv.before("PUT", target, (req) => {
      logConfig.debug('on PUT non-draft attachment');
    });
    srv.prepend(() => {
      srv.on("PUT", target, async (req, next) => {
        logConfig.debug('on PUT non-draft attachment');

        const urlWithoutParams = stripQueryParams(req?.req?.url);

        if (urlWithoutParams?.endsWith("/content")) {
          logConfig.debug('PUT request for content - passing through');
          return next();
        } else {
          logConfig.debug('PUT request for metadata - updating database record');
          const cqn = cds.odata.parse(urlWithoutParams, { service: srv })
          const extractedInfo = extractIdFromCqn(cqn)
          const id = extractedInfo.id
          await this.update(req.target, id, req.data);
        }
      });
    });
    srv.after("PUT", target, async (_, req) => {
      logConfig.debug('after PUT non-draft attachment');
