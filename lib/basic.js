...
module.exports = class AttachmentsService extends cds.Service {
  ...
  async nonDraftHandler(req, attachment, srv) {
    ...
    if (urlWithoutParams?.endsWith("/content")) {
      ...
      const existingRecord = await SELECT.from(attachment, queryKey).columns("filename", "mimeType", "url", "ID", "up__ID")
      ...
      logConfig.debug('Retrieved existing attachment metadata', {
        attachmentId: IDval,
        actualAttachmentId: existingRecord.up__ID ?? existingRecord.ID,
      ...
      const data = { 
        ID: existingRecord.up__ID ?? existingRecord.ID,
        content: req?.req?.body?.value,
        filename: existingRecord.filename,
        mimeType: existingRecord.mimeType,
