  ...
  */
  async put(attachments, data, _content, isDraftEnabled, req) {
    ...
    try {
      ...
      // Convert streams to buffers to avoid hanging issues
      let processedContent = content
      if (content && typeof content.pipe === 'function') {
        logConfig.debug('Converting stream to buffer', { 
          filename: metadata.filename,
          hasReadableMethod: typeof content.readable === 'boolean'
        })
        ...
        processedContent = Buffer.concat(chunks)
        
        logConfig.debug('Stream converted to buffer successfully', { 
          filename: metadata.filename,
          bufferSize: processedContent.length 
        })
      }
      ...
      if (!processedContent) {
        logConfig.withSuggestion('error',
          ...
          { key: blobName, hasContent: !!processedContent })
      ...
      logConfig.debug('Uploading file to Azure Blob Storage', {
        ...
        blobName,
        filename: metadata.filename,
        contentSize: processedContent.length || processedContent.size || 'unknown'
      })

      const stored = super.put(attachments, metadata, null, isDraftEnabled)
      await Promise.all([stored, blobClient.uploadData(processedContent)])
