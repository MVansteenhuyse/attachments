...
module.exports = class AzureAttachmentsService extends require("./basic") {
  ...
  init() {
    ...
    if (!separateObjectStore) {
      ...
      this.containerClient = new ContainerClient(creds.container_uri + "?" + creds.sas_token)
  ...
  async createAzureClient(tenantID) {
    ...
    try {
      ...
      const containerClient = new ContainerClient(creds.container_name)
  ...
  * @inheritdoc
  */
  async put(attachments, data, _content, isDraftEnabled, req) {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        return Promise.all(
          data.map((d) => this.put(attachments, d, _content, isDraftEnabled, req))
        )
      }

      const { content = _content, ...metadata } = data
      
      // Generate special blob name using folder structure and ID#filename format
      const folderName = this.getFolderName(attachments.name)
      const blobName = `${folderName}/${metadata.ID}#${metadata.filename}`

      if (!blobName || !metadata.ID || !metadata.filename) {
        logConfig.withSuggestion('error',
          'File ID and filename are required for Azure Blob Storage upload', null,
          'Ensure attachment data includes valid ID and filename',
          { metadata: { ...metadata, content: !!content } })
        throw new Error('File ID and filename are required for upload')
      }
      ...
      if (typeof content === "string") {
        uploadPromise = blockBlobClient.upload(content, Buffer.byteLength(content), {
          blobHTTPHeaders: {
            blobContentType: metadata.mimeType,
          }
        })
        logConfig.debug('Using upload() for string content', { blobName })
      } else if (content instanceof Uint8Array || Buffer.isBuffer(content)) {
        uploadPromise = blockBlobClient.uploadData(content, {
          blobHTTPHeaders: {
            blobContentType: metadata.mimeType,
          }
        })
        logConfig.debug('Using uploadData() for buffer/Uint8Array content', { blobName })
      } else if (content && typeof content.pipe === 'function') {
        // Stream content - use uploadStream
        uploadPromise = blockBlobClient.uploadStream(content, 8 * 1024 * 1024, 5, {
          blobHTTPHeaders: {
            blobContentType: metadata.mimeType,
          }
        })
        logConfig.debug('Using uploadStream() for stream content', { blobName })
      } else {
        logConfig.withSuggestion('error',
          'Unsupported content type for Azure Blob Storage upload', null,
          'Content must be string, Buffer, Uint8Array, or readable stream',
          { contentType: typeof content, isBuffer: Buffer.isBuffer(content), hasContent: !!content })
        throw new Error('Unsupported content type for upload')
      }

      const stored = super.put(attachments, metadata, null, isDraftEnabled)
      await Promise.all([stored, uploadPromise])
  ...
  /**
   * Generates folder name from attachment entity name with special mappings
   * @param {string} attachmentEntityName - The full entity name
   * @returns {string} - The folder name to use for blob storage
   */
  getFolderName(attachmentEntityName) {
    const parts = attachmentEntityName.split('.')
    let folderName
    
    if (parts.length > 1) {
      // For multi-part names like 'com.example.incidents.Attachments', take the second-to-last part
      folderName = parts.slice(0, -1).pop().toLowerCase()
    } else {
      // For single-part names like 'Attachments', use the name without the last part
      folderName = attachmentEntityName.toLowerCase().replace('attachments', '').replace(/\.$/, '') || 'attachments'
    }
    
    // Special folder name mappings
    if (folderName === "activevendorpromo" || folderName === "defaultvendorpromo") {
      folderName = "promo"
    }
  ...
  */
  async get(attachments, keys) {
    ...
    try {
      ...
      const response = await SELECT.from(attachments, keys).columns("filename")

      if (!response?.filename || (!keys.ID && !keys.up__ID)) {
        logConfig.withSuggestion('warn',
          'File ID or filename not found in database', null,
          'Check if the attachment exists and has been properly uploaded',
          { keys, hasResponse: !!response, hasFilename: !!response?.filename })
        return null
      }

      // Generate special blob name using folder structure and ID#filename format
      const folderName = this.getFolderName(attachments.name)
      const blobName = `${folderName}/${keys.ID ?? keys.up__ID}#${response.filename}`
      ...
      if (!downloadResponse?.readableStreamBody) {
        logConfig.withSuggestion('error',
          'No readable stream returned from Azure Blob Storage download', null,
          'Check if the blob exists and is accessible',
          { fileId: keys.ID ?? keys.up__ID, containerName: this.containerName, blobName })
        throw new Error('Failed to obtain readable stream from Azure Blob Storage')
      }
      ...
      logConfig.withSuggestion('error',
        ...
        suggestion,
        { fileId: keys?.ID ?? keys?.up__ID, containerName: this.containerName, attachmentName: attachments.name, duration })
        
      error.details = { attachmentName: attachments.name }
      throw error
    }
  }
  ...
  async updateContentHandler(req, next) {
    ...
    if (req?.data?.content) {
      const response = await SELECT.from(req.target, { ID: targetID }).columns("filename")
      if (response?.filename) {
        // Generate special blob name using folder structure and ID#filename format
        const folderName = this.getFolderName(req.target.name)
        const blobName = `${folderName}/${targetID}#${response.filename}`
        const blobClient = this.containerClient.getBlockBlobClient(blobName)
  ...
  async deleteInfectedAttachment(Attachments, key) {
    const response = await SELECT.from(Attachments, key).columns('filename')
    if (response?.filename && key.ID) {
      // Generate special blob name using folder structure and ID#filename format
      const folderName = this.getFolderName(Attachments.name)
      const blobName = `${folderName}/${key.ID}#${response.filename}`
      return await this.delete(blobName)
    }
    return false
  }
}
