...
/**
 * Extracts the ID value from a CQN where clause, handling both single and many attachment scenarios
 * @param {Object} cqn - The parsed CQN object from cds.odata.parse
 * @returns {Object} - Object containing { id, isSingleAttachment } or null if not found
 */
function extractIdFromCqn(cqn) {
  if (!cqn?.SELECT?.from?.ref) return null
  
  const ref = cqn.SELECT.from.ref
  
  // Case 1: Many attachments scenario
  // Structure: ["attachments", { where: [..., {ref: ["ID"]}, "=", {val: "attachment-id"}, ...] }]
  // Or: [{id: "entity", where: [...]}, "attachments", { where: [...] }]
  const lastRef = ref.at(-1)
  if (lastRef?.where) {
    const whereClause = lastRef.where
    const IDval = whereClause.find((r, idx) => 
      r.val && 
      whereClause[idx-1] === '=' && 
      whereClause[idx-2]?.ref?.[0] === 'ID'
    )
    if (IDval?.val) {
      return { id: IDval.val, isSingleAttachment: false }
    }
  }
  
  // Case 2: Single attachment scenario  
  // Structure: [{id: "entity", where: [{ref: ["ID"]}, "=", {val: "parent-id"}]}, "attachments"]
  // In this case, the parent ID should be used to query up__ID field
  if (ref.length >= 2 && (ref[1] === "attachments" || ref[1] === "image") && ref[0]?.where) {
    const whereClause = ref[0].where
    const IDval = whereClause.find((r, idx) => 
      r.val && 
      whereClause[idx-1] === '=' && 
      whereClause[idx-2]?.ref?.[0] === 'ID'
    ...
    if (IDval?.val) {
      return { id: IDval.val, isSingleAttachment: true }
    }
  }
  
  // Case 3: Fallback - check direct where clause on from
  if (cqn.SELECT.from.where) {
    const whereClause = cqn.SELECT.from.where
    const IDval = whereClause.find((r, idx) => 
      r.val && 
      whereClause[idx-1] === '=' && 
      whereClause[idx-2]?.ref?.[0] === 'ID'
    ...
    if (IDval?.val) {
      // Default to many attachments scenario for fallback
      return { id: IDval.val, isSingleAttachment: false }
    }
  }
