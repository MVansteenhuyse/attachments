 ...
 * @returns {string|null} - The extracted ID value or null if not found
 */
function extractIdFromCqn(cqn) {
  if (!cqn?.SELECT?.from?.ref) return null
  
  // Try different locations where the where clause might be
  const possibleWhereClauses = [
    // Case 1: Many attachments - where clause is on the last ref element
    cqn.SELECT.from.ref.at(-1)?.where,
    // Case 2: Single attachment - where clause is on the first ref element (if it's an object)
    cqn.SELECT.from.ref[0]?.where,
    // Case 3: Direct where clause on from
    cqn.SELECT.from.where
  ].filter(Boolean)
  
  // Search through each possible where clause
  for (const whereClause of possibleWhereClauses) {
    if (!Array.isArray(whereClause)) continue
    
    // Find ID value in the where clause
