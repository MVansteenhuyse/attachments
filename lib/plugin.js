...
const { stripQueryParams } = require('./helper')
...
cds.once("served", async function registerPluginHandlers() {
  ...
  async function readAttachment([attachment], req) {
    const urlWithoutParams = stripQueryParams(req?.req?.url)
    if (req._.readAfterWrite || !urlWithoutParams?.endsWith("/content") || !attachment || attachment?.content) return;
    let keys = req.data.up__ID !== undefined ? { up__ID: req.data.up__ID } : { ID: req.data.ID ?? req.params.at(-1).ID }
    let { target } = req
